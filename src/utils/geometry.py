...
def pick_xy(lms, i):
    ...
    if isinstance(lms, list) and isinstance(lms[0], dict):
        p = lms[i]
        return (p["x"], p["y"])
    # Handle direct access if lms is already formatted differently (e.g. from backend)
    # Backend lms is [{"x":..., "y":...}] so same structure.
    # If lms is from MediaPipe directly? Usually convert to dict first.
    p = lms[i]
    return (p["x"], p["y"])
